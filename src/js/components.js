...
async function cargarComponente(element, ruta) {
    ...
    try {
        const response = await fetch(`/src/components/${ruta}`);
        if (!response.ok) {
...
function resaltarNavActivo() {
    ...
    links.forEach(link => {
        ...
        if (ruta.endsWith(href) || ruta.includes(href.replace("/src/pages/", ""))) {
...
function actualizarHeaderSegunRol() {
    ...
    const adminLink = document.querySelector('.main-nav a[href="/src/pages/admin.html"]');
